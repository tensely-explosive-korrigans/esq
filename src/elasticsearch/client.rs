...
impl ElasticsearchClient {
    ...
    pub fn create_pit(&mut self) -> Result<(), ESQError> {
        let pit_response = add_auth(
            self.client.post(format!("{}/{}/_pit?keep_alive=1m", self.config.default.url, self.index.as_ref().unwrap())),
            &self.config
        ).send()?
        .json::<Value>()?;
    ...
    pub fn search(&self, query: &Value) -> Result<Value, ESQError> {
        ...
        let url = if self.pit_id.is_some() {
            ...
            format!("{}/{}/_search", self.config.default.url, self.index.as_ref().unwrap())
        };
    ...
    pub fn list_indices(&self) -> Result<Vec<Value>, ESQError> {
        let url = format!("{}/_cat/indices?format=json", self.config.default.url.trim_end_matches('/'));
